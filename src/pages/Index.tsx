...
const Index = () => {
  ...
  const handleSend = async () => {
    ...
    if (!ai) {
      alert("VITE_GEMINI_API_KEY ayarlanmamış. Lütfen projeye Gemini API anahtarınızı ekleyin.");
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-gray-800 overflow-x-hidden" style={{
      background: `
        radial-gradient(800px 500px at 12% 10%, var(--bg1, #e0e7ff), transparent 60%),
        radial-gradient(900px 600px at 88% 20%, var(--bg2, #dbeafe), transparent 62%),
        radial-gradient(900px 650px at 40% 95%, var(--bg3, #ede9fe), transparent 60%),
        linear-gradient(135deg, var(--bg1, #e0e7ff), var(--bg2, #dbeafe), var(--bg3, #ede9fe))
      `
    }}>
      {/* Header */}
          ...
          {!currentScenario ? (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
                {[
                  { id: 'job', name: 'İş Mülakatı', icon: <Briefcase className="w-6 h-6 text-indigo-500" />, color: 'indigo', desc: 'Mülakatçı rolü, net cevap iste' },
                  { id: 'date', name: 'İlk Buluşma', icon: <Heart className="w-6 h-6 text-rose-500" />, color: 'rose', desc: 'Partner rolü, akıcı diyalog kur' },
                  { id: 'presentation', name: 'Zorlu Sunum', icon: <Mic2 className="w-6 h-6 text-emerald-500" />, color: 'emerald', desc: 'Jüri/profesör rolü, cesaret testi' }
                ].map((s) => (
                  <button
                    key={s.id}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center group-hover:scale-110 transition">
                        {s.icon}
                      </div>
                      <div>
